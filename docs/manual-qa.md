...
- [ ] Details first show the selected provider's token use by model without a duplicate API-price column. The cost analytics then show the estimate, provider comparison, price coverage, model spend bars, and a daily chart when the collector supplies `byDay` data; the single partial-estimate disclosure stays neutral and says it is not a bill or subscription price.
