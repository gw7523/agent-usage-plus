...
- [ ] With "Color-code quota meters" off (default), check normal and Critical
  meter colors follow the current Omarchy theme after a theme change. Warn is
  intentionally fixed amber `#F2B705`. Enable the option and confirm both bar
  and panel use green for Healthy, amber for Warn, and red for Critical.
...
- [ ] Compact view prioritizes error/help, limits or balance, daily tokens, then model tokens. It should not show API cost, open scrolled partway down, or clip content at 1366x768 and 1920x1080.
- [ ] Details first show the selected provider's token use by model without a duplicate API-price column. `Plan vs API` then shows every enabled provider's subscription quota or prepaid API balance beside its published-rate equivalent; `—` means no exact token pricing, not zero. `Avg / recorded day` works from `activeDays` when `byDay` is unavailable, and a daily chart appears only when the collector supplies real `byDay` data. The partial disclosure stays neutral and says the equivalent is not a bill.
...
- [ ] Toggle "Color-code quota meters": it defaults off, survives
  `omarchy restart shell`, and changes both bar and panel meters together. It
  must not change percentages, warning thresholds, or notification behavior.
